...
class Game:
    ...
    def is_valid_move(self, coords: CoordPair) -> bool:
        ...
        #if coords are valid and coords are the same return true because SELF-DESTRUCT
        if coords.dst == coords.src and self.is_valid_coord(coords.src):
            return True
        ...
        if (self.is_empty(coords.dst)):
            ...
            if (self.get(coords.src).type == UnitType.AI or self.get(coords.src).type == UnitType.Tech):
                if (self.get(coords.src).player is Player.Attacker and self.get(
                    ...
                    coords.src).player is Player.Defender and self.get(coords.dst).player is Player.Defender)):
                    return True
